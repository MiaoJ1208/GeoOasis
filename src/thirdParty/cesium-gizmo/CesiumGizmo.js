...
function addMouseEvent(handler, viewer, scope) {
    ...
    handler.setInputAction(function (movement) {
        ...
        if (!scope.pickedId) {
            ...
            if (
                Cesium.defined(hovered) &&
                hovered.primitive === scope._primitive
            ) {
        ...
        const transform =
            Cesium.Transforms.eastNorthUpToFixedFrame(originPosition); // east, north, up is the positive direction of x, y, z axis
        ...
        if (scope.mode === Mode.TRANSLATE) {
            ...
            switch (scope.pickedId.type) {
                ...
                case "xyPlane":
                    ...
                    if (
                        (oldScale[0] <= minimumScale ||
                            oldScale[1] <= minimumScale) &&
                        scaleValue < 1
                    )
                        return;
